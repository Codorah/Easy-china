...
export function PremiumSection({ t }: Props) {
  const items = [
    {
      ...
      icon: (
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
          <rect x="1" y="3" width="15" height="13"/><path d="m16 8 5 0 2 4-2 4H16"/><circle cx="6" cy="19" r="2"/><circle cx="18" cy="19" r="2"/>
        </svg>
      ),
    },
    {
  ...
  return (
    <section
      className="section-py border-t border-b border-[var(--color-border)]"
      ...
      <div className="container-base">
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" role="list">
          {items.map((item) => (
            <li
              key={item.title}
              className="card p-6 relative overflow-visible flex flex-col gap-4"
            >
              {/* Tag pill */}
              <div
                className="absolute -top-3 left-5 px-3 py-1 rounded-[var(--radius-full)] text-white font-bold uppercase"
                style={{
              ...
              {/* Icon */}
              <div
                className="w-14 h-14 rounded-[var(--radius-md)] flex items-center justify-center text-[var(--color-accent)] mt-4"
                style={{ background: "var(--color-accent-soft)", border: "1.5px solid rgba(201,48,44,0.2)" }}
              >
                {item.icon}
              </div>
        ...
        <div className="text-center mt-12">
          <a
            ...
            className="inline-flex items-center gap-2 px-8 py-4 rounded-[var(--radius-md)] bg-[var(--color-accent)] text-white font-bold tracking-wide shadow-[var(--shadow-accent)] hover:bg-[var(--color-accent-strong)] transition-colors"
