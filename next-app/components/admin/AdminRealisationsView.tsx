...
const CATEGORIES = ["Import", "Ã‰tudes", "Visa", "Formation", "Tourisme", "Autre"];
...
export function AdminRealisationsView({ items, saveAction, deleteAction }: Props) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h1 className="font-display font-extrabold text-[var(--color-text)] text-[var(--text-2xl)]">RÃ©alisations</h1>
        <p className="text-[var(--color-muted)] text-[var(--text-sm)] mt-1">{items.length} rÃ©alisation{items.length !== 1 ? "s" : ""}</p>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-8 items-start">
        {/* List */}
        <section aria-label="Liste des rÃ©alisations">
          {items.length === 0 ? (
            <div className="card p-12 text-center text-[var(--color-muted)]">
              <p className="text-[var(--text-md)] font-semibold mb-2">Aucune rÃ©alisation</p>
              <p className="text-[var(--text-sm)]">Ajoutez votre premiÃ¨re rÃ©alisation avec le formulaire.</p>
            </div>
          ) : (
              ...
              {items.map((item) => (
                <li key={item.id} className="card p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[var(--color-muted)] text-[var(--text-xs)]">{item.category}{item.date ? ` Â· ${item.date}` : ""}</p>
                  </div>
        ...
        {/* Form */}
        <section className="card p-6 sticky top-8" aria-label={editing.id ? "Modifier la rÃ©alisation" : "Ajouter une rÃ©alisation"}>
          <h2 className="font-display font-bold text-[var(--color-text)] text-[var(--text-md)] mb-5">
            {editing.id ? "Modifier" : "Ajouter une rÃ©alisation"}
          </h2>
          ...
          <form action={saveFormAction} className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input id="r-title" name="title" required defaultValue={editing.title} key={editing.id + "-t"} className="field-input" placeholder="Titre de la rÃ©alisation" />
              </div>

              <div>
                <label className="field-label" htmlFor="r-cat">CatÃ©gorie</label>
              ...
              <div className="sm:col-span-2">
                ...
                <input id="r-image" name="image" type="url" defaultValue={editing.image} key={editing.id + "-i"} className="field-input" placeholder="https://â€¦" />
              </div>

              <div className="sm:col-span-2">
                <label className="field-label" htmlFor="r-desc">Description</label>
                <textarea id="r-desc" name="description" rows={3} defaultValue={editing.description} key={editing.id + "-de"} className="field-input resize-none" placeholder="Description de la rÃ©alisation" />
              </div>

              <div className="sm:col-span-2">
                <label className="field-label" htmlFor="r-tags">Tags (sÃ©parÃ©s par virgules)</label>
